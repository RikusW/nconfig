...
gboolean bShowDisabled = 1;
gboolean bShowSkipped = 1;
...
void SetPromptState(GtkTreeStore *gts, Node *n)
{
	...
	bool bVis = 1;
	if (n->GetState() & NS_SKIPPED) {
		color = "purple";
		bVis = bShowSkipped;
	} else if (n->GetState() & NS_DISABLED) {
		color = "slategrey";
		bVis = bShowDisabled;
	}
...
bool FreeFunc(Node *n, int flags, void *pv)
{
	...
	if (n->user) {
		gtk_tree_iter_free((GtkTreeIter*)n->user);
	}
...
{
	if (r) { // if rebuilding free the old tree first
...
bool ExpanderFunc(Node *n, int flags, void *pv)
{
	...
	if (n->GetType() & NTT_PARENT && (n->GetType() != NT_MENU || ted->tv == GTK_TREE_VIEW(deptree)) && n->user) {
		if (n->GetType() == NT_CHOICEP && ted->tv == GTK_TREE_VIEW(deptree)) {
			return 1;
		}
...
void tree_row_expanded(GtkTreeView *tv, GtkTreeIter *ti, GtkTreePath *tp, gpointer ud)
{
	...
	ted.tm = tm;
	if (n) {
		n->Enumerate(ExpanderFunc, NS_T_ALL, &ted);
	}
}
...
static void tree_selection_changed_cb (GtkTreeSelection *selection, gpointer data)
{
	...
	if (gtk_tree_selection_get_selected (selection, &model, &iter)) {
		...
		if (deptree == GTK_WIDGET(gtk_tree_selection_get_tree_view(selection))) {
			return;
		}
		// --- BUILD THE DEPLIST ---
		// if rebuilding free the old iterators first
		if (nLastDep) {
			nLastDep->_Enumerate(FreeFunc, NS_ALL, 0);
		}
		// clear store
		...
		nLastDep = 0;//n->GetDepTree(); //TODO XXX FIXME
		if (nLastDep) {
...
void Edited(GtkCellRendererText *gcrt, gchar *path_string, gchar *new_text, gpointer data)
{
	...
	if (!(n->GetType() & NTT_STR)) {
		printf("Edited: Node is not of type string!\n");
		return;
	}
...
bool NotifyFunc(Node *n, int flags, void *pv)
{
	...
	if (n->GetParent(NT_DLIST) == nLastDep || n == nLastDep) {
		gts = depstore; tview = deptree;
	}
...
{
	if (lfrom) {
		FileLines[iFileStrs] = lfrom; // set the from line
	}
	if (bFileNP) {
		bFileNP = 0;
		return;
...
bool ShowText(char *pp, int line)
{
	...
	if (!gtm) {
		gtm = gtk_text_buffer_create_mark(buffer, "cursor-loc-y", &ei, true);
	} else {
		gtk_text_buffer_move_mark(buffer, gtm, &ei);
	}
...
gboolean CheckForLink(GtkWidget *tv, GdkEvent *event1, gpointer ud)
{
	...
	if (event1->type != GDK_BUTTON_RELEASE) {
		return 0;
	}
	...
	int x, y;
	GtkTextIter iter;
	gtk_text_view_window_to_buffer_coords(GTK_TEXT_VIEW(tv), GTK_TEXT_WINDOW_WIDGET, (int)event->x, (int)event->y, &x, &y);
	...
	// check for link
	if (!(p = nr->GetLinkFileH(c, index, 0, 0, &l, &fn))) {
		return 0;
	}
...
void ShowHelp(Node *n)
{
	...
	if (bShowFile && ShowFile(n->GetSource(), n->GetLine())) {
		return;
	}
...
void LoadFunc(GtkMenuItem *mi, gpointer ud)
{
	nr->Load(0);
}

void SaveFunc(GtkMenuItem *mi, gpointer ud)
{
	nr->Save(0);
}

void ClearFunc(GtkMenuItem *mi, gpointer ud)
{
	nr->Load("");
}
...
bool UpdateFunc(Node *n, int flags, void *pv)
{
	...
	if (flags & NS_SKIPPED) {
		gtk_tree_store_set(store, iter, VISIBLE_COLUMN, bShowSkipped, -1);
	} else if (flags & NS_DISABLED)
	{
		gtk_tree_store_set(store, iter, VISIBLE_COLUMN, bShowDisabled, -1);
	}
	return 1;
}
...
{
	GtkPaned *pd = GTK_PANED(gtk_widget_get_parent(gtk_widget_get_parent(deptree)));
	if (!pd) {
		return;
	}
...
void SearchDlgRes(GtkButton *btn, gpointer a1) //GtkDialog *d, gint a1, gpointer ud)
{
	...
	switch ((intptr_t)a1) {
	case 1: //prompt
		if (!sn->Search(PromptSf, (char*)txt)) {
			sn = nr;
		}
		break;
	case 2: //CONFIG_
		if (!sn->Search(ConfigSf, (char*)txt)) {
			sn = nr;
		}
		break;
	case 3: //reset
...
void SearchFunc(GtkMenuItem *mi, gpointer ud)
{
	...
	GtkWidget *d = gtk_dialog_new_with_buttons("Search", GTK_WINDOW(app), GTK_DIALOG_DESTROY_WITH_PARENT, 0);

	GtkWidget *te = gtk_entry_new_with_max_length(100);
	gtk_widget_show(te);
	gtk_box_pack_start(GTK_BOX(GTK_DIALOG(d)->vbox), te, 0, 0, 0);

	GtkBox *box = GTK_BOX(GTK_DIALOG(d)->action_area);

	GtkWidget *b1 = gtk_button_new_with_mnemonic("_Prompt");
	gtk_widget_show(b1);
	gtk_box_pack_end(box, b1, 0, 0, 0);
...
void ExitFunc(GtkMenuItem *mi, gpointer ud) {
	TestExit(0, 0, 0);
}
