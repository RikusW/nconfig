...
#ifndef NC24
int CfgFile::Parse(NodeParent *np) { return 0; }
...
// if
		...
		if(!StrCmpAdv(LineStart,"if",2)) {
			...
			while(LineCurrent < LineNext && *LineCurrent != ']') LineCurrent++;
			if(*LineCurrent!=']') {
				ParseError("] expected after if [."); return 0;
			}
			*LineCurrent++=0;
...
NodeSymbols::~NodeSymbols()
{
	...
	if(path) free(path);
	for(int i=5; i<Count; i++) free(syms[i]);
}
...
if(!strncmp(s,"CONFIG_",7)) // make the parse +-3 times faster
{
	for(int i=Count-1; i>4; i--)
	{
		...
		if(!*s1 && !*s2)
		{
			if(v)
			{
				if(dep[i] & 0x80000000) dep[i]|=0x40000000;
				val[i]=v; dep[i]|=0x80000000;
			} else { val[i]=1; dep[i]++; }
			return i;
		}
	}
	...
	for(int i=Count-1; i>3; i--)
	{
		...
		if(!*s1 && !*s2)
		{
			if(v) { val[i]=v; dep[i]|=0x80000000; } else { val[i]=1; dep[i]++; }
			return i;
		}
	}
}
...
{
	if(s>=Count) return 0;
	if(dep[s] & 0x40000000) return 1;
	return 0;
}

void NodeSymbols::UnloadHelp()
{
	delete HelpFile; HelpFile = 0;
	for(int i=0; i<SYMBOL_LIMIT; i++) help[i]=0;
}

char *NodeSymbols::GetHelp(int s)
{
	if(!s) return 0; //"No help available."; // prevent unneeded load
	if(!HelpFile)
	{
	...
	if(s>=Count) return "Invalid query."; // shouldn't ever happen...
	return help[s];
}
...
{
	if(!Open("Documentation/Configure.help")) return 0;
	...
	while(LineNext < EndMem)
	{
		...
		while(1) // get a line
		{
			if(LineNext == EndMem) { LineEnd = LineNext; break; }
			if(*LineNext++ == '\n'){ LineEnd = LineNext - 1; break; }
		}
...
char *Node::ParseSymbol(char *s) { return 0; }
char *Node::ParseWord(char *s)   { return 0; }
void  Node::DefPrompt() {}
...
char *Node::ParsePrompt(char *s)
{
	...
	stop=*s++; if(!Symbols->bPromptSpaces) TRIMs b=s;
...
bool NodeDep::AddDep(int i)
{
	if(!i) return 0;
	DepList[DepCount++]=i;
	if(DepCount > 9) { printf("Error depcount exceeded."); return 0; }
	return 1;
}
...
bool NodeChoiceP::Parse(char *s,int l)
{
	...
	while(1)
	{
		TRIMs if(!*s) break; w1=s-1; *w1='\''; // prefix with \'
		NEXTs *(s-1)='\'';
		TRIMs
		NEXTs // <'prompt'   symbol>
...
bool NodeChoiceNP::Parse(char *s,int l)
{
	...
	while(1)
	{
		TRIMs if(!*s) break; tmp=s;
		NEXTs *(s-1)=' '; // gotcha! remove NEXTs null terminator
		TRIMs
		NEXTs // <'prompt'   symbol>
...
bool NodeRoot::Load(const char *ConfigFile)
{
	...
	if(bLkc26)
	{	// no segv's here!
		if(!ConfigFile || !*ConfigFile) ConfigFile = 0;
...
_if_:		if(cfile.LoadSymbols(Symbols))
	...
	if(ret) {
		if(Child) { Child->Update(0x19); Child->Update(1); } // init + normal
		Symbols->bModified = false;
	}
#endif //NC24
	return ret;
...
int CfgFile::LoadSymbols(NodeSymbols *Symbols)
{
	...
	while(ReadLine())
	{
		...
		while(LineCurrent<LineNext)
		{
			...
			if(*LineCurrent == 0) { printf("Load: = expected.\n"); return 0; }
			LineCurrent++;
		}
...
{
	if(Child && i&1) { Child->Update(1); if(bLkc26) Child->Update(1); }
...
void NodeIf::Update(int i)
{
	...
	if(i & 1) {// update down ?
		if(i & 8) {// skip/unskip
			if(i & 0x10) {// initializing ?
				if(i & 4) { // skipping ?
					...
					if(Child) Child->Update(0x1D);
					if(Else) Else->Update(0x1D);
				}else if(f) {
					if(Else) Else->Update(0x1D);
					if(Child) Child->Update(0x19);
				}else{
					if(Child) Child->Update(0x1D);
					if(Else) Else->Update(0x19);
				}
				prev_f = f;
			}else
			if(i & 4) { // skipping ?
				Notify(NS_SKIP);
				if(prev_f) { if(Child) Child->Update(0xD); }
				else       { if(Else) Else->Update(0xD); }
			}else{
				Notify(NS_UNSKIP);
				if(f) { if(Child) Child->Update(0x9); } 
				else  { if(Else) Else->Update(0x9); }
				prev_f = f;
			}
			if(Next) Next->Update(i & ~2);
			return;
		...
		if(f == prev_f)
		{
			if(f) { if(Child) Child->Update(1); } else { if(Else) Else->Update(1); }
		}else{
			if(f) { if(Else) Else->Update(0xD); if(Child) Child->Update(0x9); } else
				{ if(Child) Child->Update(0xD); if(Else) Else->Update(0x9); }
		}
		prev_f = f;
	}
...
uintptr_t NodeDep::CheckDep(uintptr_t w)
{
	...
	for(int i=0; i<DepCount; i++)
	{
		...
		if(!s) return 1; // n
		if(s>3) return 1; // n if string
		if(s==2)
		{
			if(type & NTT_MBOOL) s = 3; else // mbool treats m as y
			if(type & NTT_BOOL) return 1;
		}
...
bool NodeIf::If()
{
	...
	for(int i=0; i<Count; i++)
	{
		...
		if(!(Cond[i].cond & 1)) b=!b; // cond == '!='

		if(pcond & 2) p = (b && p); else p = b; // <and> else <or>
		if(!(Cond[i].cond & 2) && p) return 1; // (1 <or> next) = true
...
// add the first few nodes manually
	...
	Symbols->Next = DepList; DepList->Symbols = ns; // <--- n->AddChild(DepList);
...
uintptr_t Node::Advance(int updt)
{
	...
	if(word == 2 && !Symbols->HasModules()) word = 3;
	if(++word > 3) word = 1;
	if(word == 2)
	{
...
//	if(w == word) return w; // already set to w
	if(!w) { printf("Node::Set Error: Cannot unset a node.\n"); return 0; }

	if(type & NTT_NMY) // tristate / bool
	{
		if(w > 3) w = 1; // string ? set to n
		if(w == 2) // m
		{
			if(type & NTT_BOOL) w = 3;
...
//	if(!(type & NTT_INPUT)) return 0; // will this ever happen ???
	...
	if(w == word) return w; // already set to w
	if(!w) { printf("NodeDep::Set Error: Cannot unset a node.\n"); return 0; }
...
//	if(!(type & NTT_INPUT)) return 0; // will this ever happen ???
	...
	if(!w) { printf("NodeChoice::Set Error: Cannot unset a node.\n"); return 0; }
	if(w!=3) return 0;
	// w == 3 here always...
	...
	{
		if(n==this) { n=(NodeChoice*)n->Next; continue; } // skip ourself
...
char *Node::GetSource()
{
	...
	if(n) return n->GetPrompt(); else return "No Source"; // shouldn't ever happen
	return 0;
}
...
{
	if(Child) return Symbols->GetHelp(Child->GetConfig());
		 else return Symbols->GetHelp(Config); // shouldn't happen
}

char *NodeChoiceP::GetSymbol()
{
	if(Child) return Symbols->GetSymbol(Child->GetConfig());
		 else return Symbols->GetSymbol(Config); // shouldn't happen either.
}

int NodeChoiceP::GetConfig()
{
	if(Child) return Child->GetConfig(); else return Config;
}
#endif //NC24
...
void NodeRoot::FreeH()
{
	if(!bAutoFreeH) return;
	if(pHelpMem) free(pHelpMem);
	pHelpMem = 0;
}

void NodeRoot::FreeH(char *f)
{
	if((!bAutoFreeH) && f) free(f);
}

void NodeRoot::SetAutoFreeH(bool b)
{
	if(!b) FreeH(); else pHelpMem = 0;
	bAutoFreeH = b;
}
...
//=============================================================================
{
	...
	if(!strncmp(c,LinkStr[i][1],LinkLen[i][2])) // found
	{
		*end = c + LinkLen[i][2];
		...
		if((line+x) >= *start && (line+x) < *end) // return filename
		{
			char buf[255];
			char *dt=buf;
...
void NodeDList::Update(int i)
{
	...
	if(!(s & NS_SKIP)) Notify(NS_UNSKIP);
	if(!(s & NS_DISABLE)) Notify(NS_ENABLE);
	Notify(NS_STATE);
	if((s & NS_DISABLE)) Notify(NS_DISABLE);
	if((s & NS_SKIP)) Notify(NS_SKIP);
...
NodeLkc26::NodeLkc26(NodeParent *np,struct menu *m)
{
	...
	if(np->GetType() == NT_CHOICEP) type = NTT_CHOICE; else type = 0;
	pmenu = m;
	psym = m->sym;
	...
	if(pmenu->parent) if(!menu_is_visible(pmenu->parent)) state |= NS_SKIP;
	if(!menu_is_visible(pmenu)) state |= NS_DISABLE;
	...
	{
		type = NT_COMMENT;
		if(m->list) type |= NT_PARENT; // will this ever ? happen ???
		return;
	}
...
void NodeLkc26::Update(int i)
{
	...
	if(bNew && sym_has_value(psym)) { bNew=0; SetPrompt(menu_get_prompt(pmenu)); Notify(NS_PROMPT); }
nextN:
	if(pmenu->parent) if(menu_is_visible(pmenu->parent)) Notify(NS_UNSKIP); else Notify(NS_SKIP);
	if(menu_is_visible(pmenu)) Notify(NS_ENABLE);
		else if(type & NTT_DEF) Notify(NS_SKIP); else Notify(NS_DISABLE);

next:
	if(i && Child) Child->Update(1);
